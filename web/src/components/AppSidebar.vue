  ...
  <div id="app-sidebar">
    ...
    >
      ...
      <div class="relative m-3 rounded-2xl border border-white/10 bg-white/[.07] p-3 backdrop-blur-sm">
        ...
        <a v-if="showCentralWrLink" :href="centralWrUrl" class="mt-2 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-xs font-semibold text-white/65 transition hover:bg-white/10 hover:text-white" data-testid="back-to-central-wr"><NavIcon name="home" />Central WR</a>
      </div>
    </aside>
  </div>
</template>
...
const centralWrUrl = import.meta.env.VITE_CENTRAL_WR_URL || 'http://localhost:5173'
const showCentralWrLink = computed(() => ['admin', 'super_admin'].includes(authStore.userRole?.toLowerCase()))
