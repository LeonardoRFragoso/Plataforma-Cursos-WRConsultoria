...
def _image_from_data_uri(
    ...
    *,
    ...
    try:
        ...
        # The schema validates the MIME/base64 payload before persistence. The
        # renderer still fails closed if ReportLab cannot decode the image.
...
class CertificateStudioService:
        ...
        *,
        ...
        assignment = (
            await db.execute(
                ...
                .join(
                    CertificateTemplate,
                    CourseCertificateTemplateAssignment.template_id
        ...
        if assignment:
            _assignment, template = assignment
            version = (
                await db.execute(
                    ...
                    .where(
                        ...
                        CertificateTemplateVersion.status
                        == CertificateTemplateVersionStatus.PUBLISHED,
                    )
    ...
    async def assign_template(
        ...
        *,
        ...
        # Locking the course serializes concurrent first-time assignment writes,
        # avoiding a race on the tenant/course unique constraint.
        course = (
            await db.execute(
                select(Course)
...
class CertificateStudioRenderer:
    ...
    def _page_decorator(
        canvas,
        doc,
        config: CertificateVisualConfig,
        *,
        preview: bool = False,
    ) -> None:
        _ = doc
        ...
        background = (
            ...
            if config.background_style == "WHITE"
        ...
        if preview:
            # Avoid transparency-dependent rendering so preview generation is
            # deterministic across the pinned ReportLab runtime.
        ...
        *,
        ...
        doc = SimpleDocTemplate(
            ...
            title=(
                ...
                if preview
                else f"Certificate {snapshot['certificate']['number']}"
            ),
        ...
        logo = (
            _image_from_data_uri(
                config.logo_data_uri,
                width=42 * mm,
                height=18 * mm,
            )
            if config.show_issuer_logo
            else None
        )
        second_logo = (
            _image_from_data_uri(
                config.secondary_logo_data_uri,
                width=34 * mm,
                height=16 * mm,
            )
            if config.show_secondary_logo
            else None
        )
        ...
        story.extend(
            [
                ...
                Paragraph(
                    "Certificamos que "
                    f"<b>{escape(student['full_name'])}</b> concluiu o treinamento",
                    centered,
                ),
                Spacer(1, 4 * mm),
                ...
                Paragraph(
                    ...
                    f"&nbsp;|&nbsp; Carga horária: "
                    f"{escape(str(course['workload_hours']))} h "
                ...
                Paragraph(
                    ...
                    f"<b>{escape(compliance['regulatory_standard'])}</b> — versão "
                    f"{escape(str(compliance['regulatory_version']))}",
                    centered,
                ),
                Spacer(1, 4 * mm),
                Paragraph(
                    f"Período: {escape(class_data['start_date'])} a "
        ...
        details = Paragraph(
            ...
            "Código de validação: "
            f"<b>{escape(certificate['validation_code'])}</b><br/>"
        ...
        cells = (
            [drawing, details]
            if config.qr_position == "LEFT"
            else [details, drawing]
        )
        info = Table(
            [cells],
            colWidths=(
        ...
        if config.show_verification_seal:
            story.extend(
                [
                    Spacer(1, 3 * mm),
                    Paragraph(
                        "✓ Documento verificável por QR e código público",
                        centered,
                    ),
                ]
            )
        story.extend(
            [
                ...
                Paragraph(
                    ...
                    "Ledger SHA-256: "
                    f"{snapshot['training_evidence']['ledger_sha256']}",
                    small,
                ),
                PageBreak(),
        ...
        story.append(
            Paragraph(
                ...
                "Qualificação: "
                f"{escape(str(technical.get('qualification') or '-'))}<br/>"
                "Registro: "
                f"{escape(str(technical.get('professional_registration') or '-'))} "
        ...
        if instructors:
            ...
            story.append(
                Paragraph("Nenhum instrutor adicional registrado.", body)
            )
            ...
            story.extend(
                [
                    ...
                    Paragraph(
                        ...
                        f"{item['minimum_score']} — concluída em "
                        f"{escape(str(item['completed_at']))}.",
                        body,
                    ),
                ]
            )
        ...
        template = snapshot.get("certificate_template") or {}
        story.extend(
            [
                ...
                Paragraph(
                    "Template visual: "
                    f"{escape(str(template.get('template_name') or 'System Default'))} — "
                    f"versão {escape(str(template.get('version') or 1))}. "
                    "Config SHA-256: "
                    f"{escape(str(template.get('visual_config_sha256') or '-'))}",
                    small,
                ),
                Paragraph(
                    "O conteúdo regulatório desta página é gerado exclusivamente a "
                    "partir do snapshot imutável; o Certificate Studio altera somente "
                    "a camada visual.",
                    small,
                ),
            ]
        )
        callback = lambda canvas, document: cls._page_decorator(
            canvas,
            document,
            config,
            preview=preview,
    ...
    def preview(cls, visual_config: CertificateVisualConfig | dict) -> bytes:
        ...
        snapshot = {
            ...
            "certificate": {
                "number": "PREVIEW-0001",
                "validation_code": "PREVIEW-NO-VALIDITY",
                "version": 1,
                "issued_at": "2026-08-27T12:00:00",
                "expires_at": None,
            },
            "issuer": {
                "name": "Empresa de Treinamentos",
                "cnpj": "00.000.000/0000-00",
            },
            "student": {"full_name": "Nome de Exemplo"},
            "course": {
                "name": "Treinamento Regulatório de Exemplo",
                "code": "NR-XX",
                "workload_hours": 8,
                "modality": "EAD",
            },
            ...
            "compliance": {
                "regulatory_standard": "NR-XX",
                "regulatory_version": "exemplo",
            },
            "pedagogical_project": {
                "syllabus": [
                    "Conteúdo programático de exemplo",
                    "Procedimentos seguros",
                    "Avaliação e encerramento",
                ]
            },
            "technical_responsible": {
                "full_name": "Responsável Técnico de Exemplo",
                "qualification": "Qualificação de exemplo",
                "professional_registration": "REG-0000",
                "council": "CONSELHO",
                "registration_state": "UF",
            },
            "instructors": [
                {
                    "full_name": "Instrutor de Exemplo",
                    "qualification": "Qualificação de exemplo",
                }
            ],
            "assessment": {
                "score": 90,
                "minimum_score": 60,
                "completed_at": "2026-08-27T11:30:00",
            },
            "practical_component": None,
            "student_confirmation": {
                "declaration_version": "preview-v1",
                "auth_method": "PASSWORD_RECONFIRMATION",
                "accepted_at": "2026-08-27T11:35:00",
            },
            "training_evidence": {"ledger_sha256": "0" * 64},
            "validation_url": (
                "https://example.invalid/validar-certificado?code=PREVIEW-NO-VALIDITY"
            ),
        }
...
class StudioCertificateDocumentService(CertificateDocumentService):
    """Trusted document pipeline with a frozen Certificate Studio snapshot."""
    ...
    def render_original_pdf(snapshot: dict, *, snapshot_sha256: str) -> bytes:
        return CertificateStudioRenderer.render(
            snapshot,
